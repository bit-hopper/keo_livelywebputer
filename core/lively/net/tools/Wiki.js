  ...
  .toRun(function () {
    ...
    lively.BuildSpec(
      ...
      lively.BuildSpec("lively.morphic.tools.MenuBarEntry").customize({
        ...
        style: lively.lang.obj.merge(
          lively.BuildSpec("lively.morphic.tools.MenuBarEntry").attributeStore
            .style,
          {
        ...
        update: function update() {
          ...
          $world.getUserName(true, function (err, user) {
            try {
              ...
              console.error("[LoginInfo update] Error:", e.message);
            }
          });
        },

        onLoad: function onLoad() {
          this.update.bind(this).delay(0.4);
          this.startStepping(10 * 1000, "update");
        },
    	...
    	font-weight: bold;\n\
          ...
          actionChangeEmail: function actionChangeEmail() {
            ...
            Functions.composeAsync(
              function (next) {
                $world.passwordPrompt(
                  "enter password for " + context.user.name,
                  function (input) {
                    context.morph.httpCheckPassword(
                      context.user.name,
                      input,
                      next,
                    );
              ...
              function (matches, next) {
                $world.prompt(
                  "enter new email for " + context.user.name,
                  function (input) {
              ...
              function (email, next) {
                context.morph.httpModifyUser({ email: email }, next);
              },
            )(function (err, data) {
              if (err) {
          ...
          actionChangeUser: function actionChangeUser() {
            ...
            $world.askForUserName(null, function (newUsername) {
              context.morph.update(function () {
                if (newUsername) context.morph.uiInform("Username changed!");
              });
            });
          },
          actionChangePassword: function actionChangePassword() {
            ...
            Functions.composeAsync(
              function (next) {
                $world.passwordPrompt(
                  "enter current password for " + context.user.name,
                  function (input) {
                    context.morph.httpCheckPassword(
                      context.user.name,
                      input,
              ...
              function (password1, next) {
                $world.passwordPrompt(
                  ...
                  function (password2) {
                    next(
                      password1 !== password2
                        ? "repeated password does not match initial input"
                        : null,
                      password2,
                    );
                  },
              ...
              function (password, next) {
                context.morph.httpModifyUser({ password: password }, next);
              },
            )(function (err, data) {
              if (err) {
                context.morph.uiShowError(String(err));
                ...
                context.morph.update(function () {
                  context.morph.uiInform("password changed");
                });
              }
            });
          },
          actionJoinGroup: function actionJoinGroup() {
            ...
            Functions.composeAsync(
              function (next) {
                $world.prompt("Which group to join?", function (input) {
                  if (!input || !input.length)
                    context.morph.uiShowError("Invalid input " + input);
              ...
              function (groupName, next) {
                ...
                context.morph.httpModifyUser(
                  { custom: context.user.custom },
                  next,
                );
              },
            )(function (err, data) {
          ...
          actionLeaveGroup: function actionLeaveGroup() {
            ...
            Functions.composeAsync(
              function (next) {
                $world.confirm(
                  "Do you really want to leave the group " + context.group,
                  function (input) {
                    if (!input)
                      context.morph.uiInform(
                        "Not leaving group " + context.group,
                      );
                    else next();
                  },
                );
              ...
              function (next) {
                var groups = (context.user.custom.groups || []).without(
                  context.group,
                );
          ...
          actionLogoutAndExit: function actionLogoutAndExit() {
            ...
            Functions.composeAsync(
              function (next) {
                $world.confirm(
                  "Are you sure to close this world? Unsaved changes will be lost.",
                  function (ok) {
                    if (!ok) {
          ...
          actionShowGroupMembers: function actionShowGroupMembers() {
            var context = this;
            Functions.composeAsync(function (next) {
              context.morph.httpGetGroupMembers(context.group, next);
          ...
          actionShowGroupResources: function actionShowGroupResources() {
            var context = this;
            Functions.composeAsync(function (next) {
              context.morph.httpGetGroupResources(context.group, next);
              ...
              if (err) {
                ...
                context.morph.openResource(resources, "group " + context.group);
            });
          },
          actionShowMyResources: function actionShowMyResources() {
            var context = this;
            Functions.composeAsync(function (next) {
              context.morph.httpGetUserResources(context.user.name, next);
            })(function (err, resources) {
              if (err) {
                context.morph.uiShowError(String(err));
                context.morph.update.bind(context.morph).delay(5);
            ...
            Functions.composeAsync(
              function (next) {
                $world.prompt("enter user name", function (input) {
                  next(
                    !input || !input.length
                      ? new Error("not a valid user name: " + input)
                      : null,
                    input,
                  );
                });
              ...
              function (matches, next) {
                if (!matches)
                  next(new Error("Could not login as " + newUserName));
                else context.morph.httpLogin(newUserName, password, next);
              },
              function (user, next) {
                $world.setCurrentUser(user.name);
                next(null, user);
              },
            )(function (err, user) {
              if (err) {
          ...
          httpCheckPassword: function httpCheckPassword(
            userName,
            password,
            thenDo,
          ) {
            ...
            URL.root
              ...
              .withJSONWhenDone(function (json, status) {
                ...
                if (!err && !status.isSuccess())
                  err = new Error("Could not login as " + userName);
                thenDo(err, status.isSuccess());
              });
          },
          httpCurrentGroupData: function httpCurrentGroupData(thenDo) {
            var self = this;
            Global.URL.nodejsBase
              .withFilename(
                "AuthServer/groups/of-user/" + $world.getUserName(true),
              )
              .asWebResource()
              .beAsync()
              .get()
              .withJSONWhenDone(function (json, status) {
                var err = json && json.error;
                if (!err && !status.isSuccess())
                  err = new Error(json.error || "Could not get groups");
          ...
          httpGetGroupMembers: function httpGetGroupMembers(groupName, thenDo) {
            ...
            var self = this;
            Global.URL.nodejsBase
              .withFilename("AuthServer/groups/members-of/" + groupName)
              .asWebResource()
              .beAsync()
              .get()
              .withJSONWhenDone(function (json, status) {
                var err = json && json.error;
                if (!err && !status.isSuccess())
                  err = new Error(json.error || "Could not get group members");
                thenDo.call(self, err, json.users || []);
              });
          },
          httpGetGroupResources: function httpGetGroupResources(
            groupName,
            thenDo,
          ) {
            ...
            var self = this;
            Global.URL.nodejsBase
              .withFilename("AuthServer/groups/resources-of/" + groupName)
              .asWebResource()
              .beAsync()
              .get()
              .withJSONWhenDone(function (json, status) {
                ...
                if (!err && !status.isSuccess())
                  err = new Error(
                    json.error ||
                      "Could not get resources of group " + groupName,
                  );
                thenDo.call(self, err, json.resources || []);
              });
          },
          httpGetUserResources: function httpGetUserResources(
            userName,
            thenDo,
          ) {
            ...
            var self = this;
            Global.URL.nodejsBase
              .withFilename("AuthServer/users/resources-of/" + userName)
              .asWebResource()
              .beAsync()
              .get()
              .withJSONWhenDone(function (json, status) {
                ...
                if (!err && !status.isSuccess())
                  err = new Error(
                    json.error || "Could not get resources of user " + userName,
                  );
                thenDo.call(self, err, json.resources || []);
              });
          },
          httpLogin: function httpLogin(name, password, thenDo) {
            ...
            URL.root
              ...
              .withJSONWhenDone(function (json, status) {
                thenDo.call(
                  self,
                  !json || json.error || !status.isSuccess()
                    ? new Error(json.error || String(status))
                    : null,
                  json,
                );
              });
          ...
          httpLogout: function httpLogout(thenDo) {
            ...
            URL.root
              ...
              .whenDone(function (_, status) {
                thenDo.call(
                  self,
                  !status.isSuccess() ? new Error(String(status)) : null,
                );
              });
          },
          ...
          openResource: function openResource(resources, title) {
            ...
            if (grouped.partsbin && grouped.partsbin.length) {
              ...
              partItems.forEach(function (ea) {
                partItemList.addMorph(ea);
              });
            ...
            if (grouped.world && grouped.world.length) {
              ...
              lively.bindings.connect(
                ...
                "selection",
                {
                  visitWorld: function (morph) {
                    $world.confirm(
                      "open world " + morph.item.value + "?",
                      function (input) {
                        if (input)
                          window.open(
                            Global.URL.root
                              .withFilename(morph.item.value)
                              .toString(),
                            "_blank",
                          );
                      },
            ...
            if (grouped.other && grouped.other.length) {
              ...
              lively.bindings.connect(
                ...
                "selection",
                {
                  open: function (morph) {
                    lively.ide.openFile(
                      Global.URL.root.withFilename(morph.item.value),
                    );
                  },
                },
                "open",
            ...
            function setStringList(listItems, list) {
              var items = listItems.map(function (path) {
                return { isListItem: true, value: path, string: path };
              });
              list.setList(items);
            ...
            function createPartItemList(partResources) {
              return partResources
                ...
                .map(function (ea) {
                  return {
                    partsSpace: ea.slice(0, ea.lastIndexOf("/")),
                ...
                .map(function (ea) {
                  var item = {
                    isListItem: true,
                    value: lively.PartsBin.getPartItem(
                      ea.name,
                      ea.partsSpace,
                    ).asPartsBinItem(),
                    string: ea.name,
          ...
          textDoit: function textDoit(actionName, context) {
            return {
              doit: {
                code: Strings.format(
                  ";(%s).call(this);",
                  this["action" + actionName.capitalize()],
                ),
                context: context,
              },
              hover: {
            ...
            this.httpCurrentUserData(function (err, data) {
              ...
              this.httpCurrentGroupData(function (groupErr, groups) {
                ...
                var markup = [
                  ...
                  ["\n\n", {}],
                  [
                    "Show resources I have created or modified",
                    this.textDoit("showMyResources", context),
                  ],
                  ["\n", {}],
                  [
                    "Open my workspace",
                    this.textDoit("openMyWorkspace", context),
                  ],
                  ["\n", {}],
                  ["\nYour groups:", {}],
                  ["\n", {}],
                  ...
                  groups.map(function (group) {
                    return [
                      ...
                      [
                        ...
                        this.textDoit(
                          "showGroupMembers",
                          Object.extend({ group: group }, context),
                        ),
                      ],
                      [" ", {}],
                      [
                ...
                markup.push([
                  "Join group",
                  this.textDoit("joinGroup", context),
                ]);
          ...
          validateEmail: function validateEmail(email, thenDo) {
            ...
            thenDo(
              matches ? null : new Error(email + " is not a valid email"),
              matches,
            );
          },
        },
      ...
      submorphs: [
        {
          ...
          reset: function reset() {
            ...
            lively.bindings.connect(
              this.get("VisitButton"),
              "fire",
              this,
              "visitVersion",
            );
            lively.bindings.connect(
              this.get("RevertButton"),
              "fire",
              this,
              "revertToVersion",
            );
          ...
          revertToVersion: function revertToVersion() {
            ...
            if (!path) {
              $world.inform("No resource selected");
              return;
            }
            var sel = this.get("VersionList").selection;
            if (!sel) {
              $world.inform("No version selected");
              return;
            }
          ...
          showResult: function showResult(err, versions) {
            ...
            var items = versions.map(function (version) {
              try {
                var date = new Date(version.date).format(
    ...
    lively.BuildSpec("lively.wiki.ToolFlap", {
      ...
      alignInWorld: function alignInWorld() {
        ...
        var topRight = pt(
          this.world().visibleBounds().width - (40 + 2 * 130),
          -10,
        );
      ...
      alignSubmorphs: function alignSubmorphs() {
        this.statusText.align(
          this.statusText.bounds().center(),
          this.innerBounds().bottomCenter().addXY(0, -8),
        );
        this.menu &&
          this.menu.align(
      ...
      collapse: function collapse() {
        // this.collapse()
        this.withCSSTransitionForAllSubmorphsDo(
          function () {
      ...
      expand: function expand() {
        ...
        var items = [
          [
            "world versions",
            function () {
              var versionViewer = lively
                .BuildSpec("lively.wiki.VersionViewer")
                .createMorph()
                .openInWorldCenter();
      ...
      onMouseDown: function onMouseDown(evt) {
        if (
          evt.getTargetMorph() !== this.statusText &&
      ...
      reset: function reset() {
        ...
        this.statusText = lively.morphic.Text.makeLabel("Wiki", {
          align: "center",
