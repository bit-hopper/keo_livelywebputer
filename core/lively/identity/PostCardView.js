...
module('lively.identity.PostCardView')
  ...
  .toRun(function () {
    ...
    var PostCardViewClass = lively.morphic.Box.subclass('lively.identity.PostCardView',

    'serialization', {
      doNotSerialize: [
        '_wrapperEl', '_cardEl', '_frontEl', '_backEl',
        '_avatarImgEl', '_titleEl', '_contentEl', '_stampEl',
        '_didEl', '_cidEl', '_dateEl', '_visibilityEl', '_verifyBadgeEl',
        '_editBtn',
      ],
    },
    ...
    'chrome', {
      ...
      _buildChrome: function () {
        ...
        wrapper.style.cssText = 'position:absolute;inset:0;perspective:1200px;';
      ...
      _buildFace: function (card, isBack) {
        ...
        face.className = 'lively-postcard-view-face ' + (isBack ? 'back' : 'front');
        face.style.cssText = [
      ...
      _buildFrontContents: function (front) {
        ...
        avatar.style.cssText = [
          ...
          'border-radius:50%', 'box-shadow:0 0 0 2px #fff, 0 1px 3px rgba(0,0,0,0.3)',
        ].join(';');
        ...
        title.style.cssText = [
          'position:absolute', 'top:12px', 'left:52px', 'right:40px',
        ...
        content.style.cssText = [
          ...
          'padding:8px 14px 14px', 'overflow-y:auto', 'font-size:13px',
          'line-height:1.5', 'color:#333', 'box-sizing:border-box',
        ].join(';');
        front.appendChild(content);
        this._contentEl = content;

        var flipBtn = this._buildIconButton('front', '⟳', 'Flip to see verification info', function () {
          self._toggleFlip();
        });
        ...
        ['mousedown', 'click'].forEach(function (t) {
          editBtn.addEventListener(t, function (e) {
            ...
            if (t === 'click') lively.identity.PostCardEditor.openCard(self._handle, self._objId);
          });
        });
      ...
      _buildBackContents: function (back) {
        ...
        stamp.style.cssText = [
          ...
          'border:2px dashed currentColor', 'border-radius:3px',
        ...
        meta.style.cssText = [
          'position:absolute', 'top:16px', 'left:14px', 'right:68px', 'bottom:14px',
        ...
        badge.style.cssText = [
          'position:absolute', 'left:14px', 'bottom:38px', 'font-size:12px', 'font-weight:600',
        ].join(';');
        badge.textContent = 'Checking…';
        back.appendChild(badge);
        this._verifyBadgeEl = badge;

        var flipBackBtn = this._buildIconButton('back', '⟲', 'Flip back', function () {
          self._toggleFlip();
        });
    ...
    'rendering', {
      ...
      _renderEnvelope: function (envelope) {
        ...
        this._avatarImgEl.src = lively.identity.postCardUtils.identiconDataUrl(this._handle || envelope.did, 32);
        this._titleEl.textContent = (envelope.state && envelope.state.title) || '(untitled)';
        if (this._editBtn) this._editBtn.style.display = this._isOwner ? '' : 'none';
      ...
      _renderContentArea: function (envelope) {
        ...
        if (envelope.visibility === 'public') {
          ...
          this._contentEl.innerHTML = snapshot ? lively.identity.postCardUtils.snapshotToHtml(snapshot) : '';
          return;
        }
        ...
        lock.style.cssText = 'display:flex;flex-direction:column;align-items:center;justify-content:center;' +
        ...
        viewBtn.style.cssText = 'font-size:11px;padding:4px 10px;cursor:pointer;' +
          'border:1px solid #ccc;border-radius:12px;background:#fff;';
        ['mousedown', 'click'].forEach(function (t) {
          viewBtn.addEventListener(t, function (e) {
            ...
            if (t === 'click') {
              lively.identity.PostCardEditor.openCard(self._handle, self._objId, { forceReadOnly: true });
            }
          });
        });
      ...
      _renderBackMeta: function (envelope) {
        this._didEl.textContent = lively.identity.postCardUtils.truncateDid(envelope.did);
        this._cidEl.textContent = (envelope.record && envelope.record.cid) ?
        ...
        var stampColor = envelope.visibility === 'public' ? '#888' : '#5566cc';
      ...
      _toggleFlip: function () {
        ...
        this._cardEl.style.transform = this._flipped ? 'rotateY(180deg)' : 'rotateY(0deg)';
      },
    ...
    'verification', {
      ...
      _verify: function (envelope) {
        ...
        function finish(signerJwk) {
          lively.identity.crypto.verifyEnvelopeIntegrity(envelope, signerJwk || null, function (err, result) {
            self._verifyResult = result;
            self._renderVerifyBadge(result || { cidValid: false, sigStatus: 'unresolved' });
          });
        }

        if (!envelope.sig) return finish(null);
        lively.identity.did.resolveEnvelopeSignerJwk(this._handle, function (err, jwk) {
          finish(err ? null : jwk);
        });
      },
      ...
      _renderVerifyBadge: function (result) {
        var label, color;
        if (!result.cidValid) { label = '⚠ Content tampered'; color = '#c33'; }
        else if (result.sigStatus === 'verified') { label = '✓ Verified'; color = '#2a7'; }
        else if (result.sigStatus === 'unsigned') { label = 'Unsigned'; color = '#999'; }
        else if (result.sigStatus === 'unresolved') { label = 'Unable to verify'; color = '#999'; }
        else { label = '✕ Signature invalid'; color = '#c33'; }
    ...
    Object.extend(PostCardViewClass, {
      ...
      open: function (handle, objId, options) {
        var opts = options || {};
        var view = new lively.identity.PostCardView(opts.bounds || lively.rect(0, 0, 420, 300));
        view._handle = handle;
        view._objId = objId;
        ...
        if (opts.target) {
          ...
          this._openInCenteredWindow(view, 'Post Card');
          view._setup();
        }
        return view;
      },
