module("lively.morphic.MorphAddons")
  .requires(
    "lively.morphic.Core",
    "lively.morphic.Events",
    "lively.morphic.Widgets",
    "lively.morphic.Styles",
  ...
  .toRun(function () {
    ...
    Object.extend(lively.morphic, {
      showMarkerFor: function (morph) {
        ...
        var m =
          lively._showMarker ||
          (lively._showMarker = $world.loadPartItem(
            "Marker",
            "PartsBin/Basic",
          ));
        m.setOpacity(1);
        ...
        (function () {
          m.setOpacityAnimated(0, 1000, function () {
            m.remove();
          });
        }).delay(1);
      },
      ...
      show: function (obj) {
        ...
        function newShowPt(/*pos or x,y, duration, extent*/) {
          ...
          var pos =
              args[0].constructor == lively.Point
                ? args.shift()
        ...
        function newShowMorph(morph) {
          return newShowRect(
            morph.getGlobalTransform().transformRectToRect(morph.innerBounds()),
          );
        }
        ...
        function newShowElement(el) {
          return lively
            .$(el)
            .bounds({ withMargin: true, withPadding: true })
            .show(2000);
        }
        ...
        function newShowThenHide(morphOrMorphs, duration) {
          ...
          var morphs = Object.isArray(morphOrMorphs)
            ? morphOrMorphs
            : [morphOrMorphs];
          duration = duration || 3;
        ...
        if (!obj && !Object.isString(obj)) {
          return lively.morphic.show(String(obj));
        ...
        else if (obj instanceof Global.Element && obj.getBoundingClientRect) {
          var b = obj.getBoundingClientRect();
          return newShowRect(lively.rect(b.left, b.top, b.width, b.height));
        } else {
          var msg = Strings.format("%o", obj);
          return showText(msg);
        }
      },
      ...
      printInspect: (function () {
        ...
        function inspectPrinter(val, ignore) {
          ...
          if (Global.ImageData && val instanceof Global.ImageData)
            return String(val);
          if (length !== undefined && length > maxColLength && val.slice) {
            var printed =
              typeof val === "string" || val.byteLength
                ? String(val.slice(0, maxColLength))
                : val.slice(0, maxColLength).map(lively.lang.string.print);
            return "[" + printed + ",...]";
          }
          return ignore;
        }
        ...
        return function (obj, maxDepth) {
          ...
          if (typeof obj === "string")
            return obj.length > maxColLength
              ? obj.slice(0, maxColLength) + "..."
              : String(obj);
    ...
    lively.morphic.Window.addMethods("undo", {
    ...
    lively.morphic.Morph.addMethods(
      "morphic relationship",
      {
        ...
        indentedListItemsOfMorphNames: function (indent) {
          ...
          if (this.name) {
            items.push({
              isListItem: true,
              string: indent + this.name,
              value: this,
              selectionString: this.name,
            });
            indent += indent;
          }
          items = items.concat(
            this.submorphs
              .invoke("indentedListItemsOfMorphNames", indent)
              .flatten(),
          );
          return items;
        },
        ...
        treeItemsOfMorphNames: function (options) {
          ...
          var item = {
              name:
                this.name ||
                "a " + lively.Class.getConstructor(this).displayName,
              value: this,
            },
            children = this.submorphs
              .invoke("treeItemsOfMorphNames", options)
              .compact();
          if (children.length > 0) item.children = children;
          lively.lang.properties.own(properties).forEach(function (v) {
            item[v] = properties[v];
          });
      ...
      "opening",
      {
        ...
        openInWindow: function (optPosOrOptions) {
          ...
          if (options.x !== undefined && options.y !== undefined)
            options = { pos: options };
          lively.morphic.World.current().internalAddWindow(
            this,
            options.title || this.name,
            options.pos || this.getPosition(),
          );
        ...
        removeAndDropSubmorphs: function () {
          ...
          if (supermorph && supermorph.isMorph) {
            ...
            throw (
              "Cannot remove " +
              this +
              " with a submorph drop. It has no owner."
            );
          }
        },
      },
        ...
        undoRedoTransformationChange: function (spec, undo) {
          if (!undo || undo == "undo") {
            ...
            if (spec.startOwner !== spec.endOwner) {
              if (spec.startOwner == null) this.remove();
              else spec.startOwner.addMorph(this);
            }
          ...
          if (undo == "redo") {
            ...
            // Change of origin by itself
            if (
              spec.startExtent.eqPt(spec.endExtent) &&
              !spec.startOrigin.eqPt(spec.endOrigin)
            ) {
        ...
        logTransformationForUndo: function (actionName, phase, evt) {
          ...
          if (phase == null || phase == "start") {
            ...
            $world.logMorphicAction({
              ...
              startExtent: this.getExtent(),
              startOwner: this.owner,
              startIndexInSubmorphs: this.owner
                ? this.owner.submorphs.indexOf(this)
                : null,
          ...
          var amendments = {
            ...
            actionName: actionName,
            phase: phase,
            endTransform: this.getTransform(),
            endExtent: this.getExtent(),
            endOwner: this.owner,
            endOrigin: this.getOrigin(),
          };

          if (this.owner)
            amendments.endIndexInSubmorphs = this.owner.submorphs.indexOf(this);
          $world.amendMorphicAction(amendments);
        },
      },
      "styling",
      {
        setPadding: function (padding) {
          ...
          console.warn(
            "DEPRECATED: setPadding called on Morph that doesn't support padding",
          );
        },
        ...
        makeStyleSpec: function () {
          ...
          if (this.shape.getBorderRadius)
            spec.borderRadius = this.shape.getBorderRadius() || 0.0;
          spec.fillOpacity =
            typeof this.shape.getFillOpacity() !== undefined
              ? this.shape.getFillOpacity()
              : 1.0;
          spec.strokeOpacity =
            typeof this.shape.getStrokeOpacity() !== undefined
              ? this.shape.getStrokeOpacity()
              : 1.0;
          return spec;
        },
        ...
        getGridPoint: function () {
          return this.owner && this.owner.layout && this.owner.layout.grid
            ? this.owner.layout.grid
            : pt(Config.get("gridSpacing"), Config.get("gridSpacing"));
        },
      },
      "layouting helpers",
      {
      ...
      "animations",
      {
        dissolve: function (ms) {
          ...
          var steps = 10,
            stepTime = ms / steps,
            opacityDelta =
              Math.max(this.getFillOpacity(), this.getOpacity()) / steps,
            morph = this;
      ...
      "fixed positioning",
      {
        enableFixedPositioning: function () {
          if (!this.owner || !this.owner.isWorld) {
            console.warn(
              "Setting fixed positioning for morph %s but owner is not world!",
              this,
            );
          }
      ...
      "fullscreen",
      {
        enterFullScreen: function (beTopLeft) {
          ...
          if (ratio > 0 && ratio < 100) {
            var newWindowExtent = pt(
              windowExtent.x / ratio,
              windowExtent.y / ratio,
            );
            ...
            if (beTopLeft) {
              ...
              this.align(
                this.worldPoint(this.shape.bounds().topCenter()),
                pt(newWindowExtent.x / 2, 0),
              );
      ...
      {
        show: function () {
          lively.morphic.show(this);
        },
        edit: function () {
          lively.morphic.edit(this);
        },
      },
      "ui messaging",
      {
    ...
    lively.morphic.Morph.addMethods("overlay", {
      addOverlay: function () {
        this.removeOverlay();
    ...
    Object.extend(lively.morphic.Morph, {
        ...
        switch (arguments.length) {
          case 1: // rectangle
            if (!(arguments[0] instanceof Rectangle))
              throw new TypeError(arguments[0] + " not a rectangle");
            bounds = arguments[0];
            break;
      ...
      makeCircle: function (location, radius, lineWidth, lineColor, fill) {
        ...
        var morph = new lively.morphic.Morph(
          new lively.morphic.Shapes.Ellipse(bounds),
        );
      ...
      makeCubicBezier: function (controlPoints, lineWidth, lineColor) {
        var ptToString = function (point) {
          return point.x + "," + point.y;
        };
        var svgDescriptors =
          "M" +
          ptToString(controlPoints[0]) +
          "C" +
          controlPoints.slice(1).map(ptToString).join(" ");
        var bezierCurve = new lively.morphic.Path([
          lively.pt(0, 0),
          lively.pt(0, 0),
        ]);

        bezierCurve.shape.setPathElements(
          lively.morphic.Shapes.PathElement.parse(svgDescriptors),
        );
    ...
    lively.Line.addMethods("conversion", {
      asMorph: function () {
    ...
    lively.morphic.World.addMethods(
      ...
      "debugging",
      {
        logError: function (er, optName) {
          Global.LastError = er;
          var msg =
            (optName || "LOGERROR: ") + String(er) + "\nstack:" + er.stack;
          this.setStatusMessage(
            msg,
            Color.red,
            10,
            function () {
              var errorStackViewer = this.openPartItem(
                "ErrorStackViewer",
                "PartsBin/Tools",
              );
              errorStackViewer.align(
                errorStackViewer.bounds().topCenter(),
                this.visibleBounds().topCenter(),
              );
      ...
      "logging",
      {
        setStatusMessage: function (msg, color, delay, callback, optStyle) {
          ...
          var msgMorph = this.createStatusMessage(
            msg,
            Object.merge([{ fill: color }, optStyle]),
          );
          // callbacks are currently not supported...
          if (false && callback) {
            var btn = new lively.morphic.Button(
              lively.rect(0, 0, 50, 20),
              "more",
            );
            ...
            btn.align(
              btn.bounds().topRight(),
              closeBtn.bounds().topLeft().addPt(pt(-5, 0)),
            );
        ...
        alert: function (msg, delay) {
          ...
          this.setStatusMessage(msg, Color.red, delay, function () {
            var w = $world.addTextWindow({
              title: "ALERT",
              content: msg,
              syntaxHighlighting: false,
            });
            w.owner.align(
              w.owner.bounds().topRight(),
              $world.visibleBounds().topRight(),
            );
          });
        },
        alertOK: function (msg, delay) {
        ...
        addStatusMessageMorph: function (morph, delay) {
          ...
          while (
            this.statusMessages.length >= Config.get("maxStatusMessages")
          ) {
          ...
          morph.addScript(function remove() {
            ...
            if (
              RealTrait &&
              this._traitConfig_ &&
              this._traitConfig_.length > 0
            ) {
              var self = this;
              for (var i = 0; i < this._traitConfig_.length; i++)
                var trait =
                  RealTrait.prototype.traitRegistry[
                    this._traitConfig_[i].traitName
                  ];
              var holder = trait.extendedObjectsAndOptions.objects.detect(
                function (e) {
        ...
        createStatusMessage: function (msg, options) {
          ...
          var textMsg = msgMorph.addMorph(
            new lively.morphic.Text(msgMorph.innerBounds().insetBy(10), ""),
          );
          ...
          var closeBtn = msgMorph.addMorph(
            new lively.morphic.Text(msgMorph.innerBounds().withWidth(30), "✗"),
          );
          closeBtn.align(
            closeBtn.bounds().topLeft(),
            msgMorph.innerBounds().topLeft(),
          );
          ...
          msgMorph.addScript(function setMessage(msg, color) {
            ...
            var cssClass =
              (Color.red.equals(color) && "failure") ||
          ...
          msgMorph.addScript(function onDoubleClick(evt) {
            ...
            (function () {
              ...
              if (ext.y > visibleBounds.extent().y)
                ext.y = visibleBounds.extent().y - 20;
              if (ext.x > visibleBounds.extent().x)
                ext.x = visibleBounds.extent().x - 20;
      ...
      "undo",
      {
        ...
        amendMorphicAction: function (amendment) {
          ...
          if (
            lastAction.morph !== amendment.morph ||
            lastAction.actionName !== amendment.actionName
          ) {
            console.log(
              "Unmatching log of state for " +
                amendment.actionName +
                " " +
                amendment.phase,
            );
          ...
          for (var p in amendment) {
            lastAction[p] = amendment[p];
          }
        },
        undoReadme: function () {
      ...
      "preferences",
      {
        openPreferences: function () {
          require("lively.morphic.tools.Preferences").toRun(function () {
            lively
              .BuildSpec("lively.morphic.tools.Preferences")
              .createMorph()
              .openInWorldCenter()
              .comeForward();
          });
        ...
        askForNewWorldExtent: function () {
          var world = this;
          // Automatically calculate the full-screen extent from current window bounds
          // windowBounds() returns the viewport dimensions the morphic world can occupy
          var windowBounds = world.windowBounds();
          // Extract the width and height from the bounds to create a full-screen extent
          var fullScreenExtent = windowBounds.extent();

          // Apply the calculated full-screen extent to the world immediately
          // This replaces the old behavior of prompting the user for manual input
          world.setExtent(fullScreenExtent);
          // Provide visual confirmation that the world now fills the entire available screen space
          alertOK(
            "World extent set to fill entire screen: " + fullScreenExtent,
          );
        },
        ...
        askForNewBackgroundColor: function () {
          ...
          world.prompt(
            "Please enter new world background color",
            function (str) {
              if (!str) return;
              var newColor;
              try {
      ...
      "auth",
      {
        askForUserName: function (prompt, thenDo) {
          ...
          var world = this,
            oldUserName = world.getUserName(true);
          world.prompt(
            ...
            function (name) {
              if (name && name.length > 0) {
                ...
                world.setStatusMessage(
                  "User name is now: " + world.getUserName(true),
                  Color.green,
                );
              } else {
                var msg = oldUserName
        ...
        showUserConfig: function () {
          ...
          lively.lang.fun.composeAsync(
            user
              ...
              : function (n) {
                  $world.askForUserName(
                    "No username set yet, please enter your username:",
                    function (input) {
                      n(null, input);
                    },
                  );
            ...
            function (username, n) {
              if (!username)
                return n(new Error("Not a valid username: " + username));
            ...
            if (err)
              $world.inform("Could not browser user config because:\n" + err);
          });
    ...
    lively.morphic.Box.subclass(
      "lively.morphic.Panel",
      "settings",
      {
        style: {
          resizeWidth: true,
          resizeHeight: true, // so it scales itself
      ...
      "initializing",
      {
        ...
        newTextPane: function (initialBounds, defaultText, style) {
          ...
          if (style) text.applyStyle(style);
          return text;
        },
        newStaticTextPane: function newStaticTextPane(
          extent,
          initialText,
          style,
        ) {
        ...
        setActive: function (button, bool) {
          button.applyStyle({
            borderColor: bool ? Color.red : Color.black,
            borderWidth: bool ? 2 : 1,
          });
        ...
        newDragnDropListPane: function (
          initialBounds,
          suppressSelectionOnUpdate,
        ) {
        ...
        arrangeElementsAccordingToSpec: function (extent, paneSpecs, optPanel) {
          ...
          paneSpecs.forEach(function (spec) {
            var paneName = spec[0],
              ...
              relativeRect =
                spec[2] instanceof lively.Rectangle
                  ? spec[2]
                  : new lively.Rectangle(
                      spec[2][0],
                      spec[2][1],
                      spec[2][2],
                      spec[2][3],
    ...
    lively.morphic.Text.addMethods("deprecated interface", {
      innerMorph: function () {
        return this;
      },
    ...
    lively.morphic.Button.addMethods("old interface", {
    ...
    lively.morphic.Morph.addMethods("CSSTransitions", {
      ...
      withCSSTransitionForAllSubmorphsDo: function (
        morphModifyFunc,
        duration,
        whenDone,
      ) {
      ...
      withCSSTransitionForAllMorphsDo: function (
        morphs,
        morphModifyFunc,
        duration,
        whenDone,
      ...
      alignAnimated: function (posA, posB, time, callback) {
        if (posA.eqPt(posB)) callback && callback.call(this);
          ...
          this.withCSSTransitionDo(
            this.align.bind(this, posA, posB),
            time,
            callback,
          );
      },
      ...
      setPositionAnimated: function (position, time, callback) {
        if (this.getPosition().eqPt(position)) callback && callback.call(this);
          ...
          this.withCSSTransitionDo(
            this.setPosition.curry(position),
            time,
            callback,
          );
      },
      ...
      addMorphWithFixedPosition: function (morph) {
        ...
        if (!this.isRendered()) {
          lively.bindings.connect(
            ...
            "_isRendered",
            this,
            "addMorphWithFixedPosition",
            {
              updater: function ($upd) {
                $upd(morph);
              },
              removeAfterUpdate: true,
              varMapping: { morph: morph },
            },
          );
          return this;
        }
        ...
        var parentRenderCtxt = newOwner.renderContext(),
          ...
          ctx =
            parentRenderCtxt.constructor !== subRenderCtxt.constructor
              ? parentRenderCtxt.newForChild()
              : subRenderCtxt;
    ...
    Trait("lively.morphic.FixedPositioning.MorphTrait", {
      addEventHandlerForFixedPositioning: function () {
        ...
        this.eventHandler.handleEvent = function (evt) {
          ...
          try {
            wasHandled =
              evt.world[eventSpec.targetMethodName] &&
              evt.world[eventSpec.targetMethodName](evt);
      ...
      setPosition: function (pos) {
        pos = this.getFixedPositionTransform(true)
          .inverse()
          .transformPoint(pos);
    ...
    lively.morphic.Text.subclass(
      ...
      "initializing",
      {
        ...
        createCloseButton: function () {
          var closeBtn = new lively.morphic.Button(
            lively.rect(0, 0, 18, 18),
            "X",
          );
          closeBtn.name = "closeButton";
          lively.bindings.connect(closeBtn, "fire", this, "remove");
          this.addMorph(closeBtn);
          closeBtn.addScript(function onMouseMove(evt) {
            this.focus();
          });
          closeBtn.addScript(function alignInOwner() {
            var offset = this.owner.showsVerticalScrollBar()
              ? this.owner.getScrollBarExtent().x + 3
              : 2;
            this.align(
              this.bounds().topRight(),
              this.owner.innerBounds().topRight().addXY(-offset, 2),
            );
          });
      ...
      "message interface",
      {
          ...
          if (!Array.isArray(msg)) {
            msg = [
              [
                ...
                {
                  color: color,
                  doit: { code: "evt.getTargetMorph().expand();" },
                },
              ],
              ["\n"],
              [String(msg)],
        ...
        expand: function (insertionMorph, textMode) {
          var ctx = lively
            .PropertyPath("textChunks.0.style.doit.context")
            .get(this);
          var content =
            (ctx && ctx.content) ||
            this.insertion ||
            this.textString.replace(/^expand\n?/, "");
      ...
      "layouting",
      {
        alignAtBottomOf: function (forMorph) {
          ...
          this.fitThenDo(function () {
            ...
            forMorph.world() &&
              this.setPosition(
                forMorph.owner.worldPoint(forMorph.bounds().bottomLeft()),
              );
    ...
    lively.morphic.Box.subclass("lively.morphic.BoundsMarker", "initializing", {
      isEpiMorph: true,
      ...
      markerLength: function (forBounds) {
        ...
        return Math.max(
          4,
          Math.floor(length / 10 < 10 ? length / 2 - 5 : length / 10),
        );
      },
      ...
      ensureMarkerCorners: function () {
        var topLeftH =
            this.topLeftH ||
            (this.topLeftH = this.addMorph(this.createMarkerEdge())),
          topLeftV =
            this.topLeftV ||
            (this.topLeftV = this.addMorph(this.createMarkerEdge())),
          topRightH =
            this.topRightH ||
            (this.topRightH = this.addMorph(this.createMarkerEdge())),
          topRightV =
            this.topRightV ||
            (this.topRightV = this.addMorph(this.createMarkerEdge())),
          bottomRightH =
            this.bottomRightH ||
            (this.bottomRightH = this.addMorph(this.createMarkerEdge())),
          bottomRightV =
            this.bottomRightV ||
            (this.bottomRightV = this.addMorph(this.createMarkerEdge())),
          bottomLeftH =
            this.bottomLeftH ||
            (this.bottomLeftH = this.addMorph(this.createMarkerEdge())),
          bottomLeftV =
            this.bottomLeftV ||
            (this.bottomLeftV = this.addMorph(this.createMarkerEdge()));
        return [
          topLeftH,
          topLeftV,
          topRightH,
      ...
      alignWithRect: function (r) {
        var corners = this.ensureMarkerCorners(),
          ...
          boundsForMarkers = [
            ...
            r
              .bottomRight()
              .addXY(-4, -markerLength)
              .extent(pt(0, markerLength)),
            r
              .bottomRight()
              .addXY(-markerLength, -2)
              .extent(pt(markerLength, 0)),
    ...
    Object.extend(lively.morphic.BoundsMarker, {
      highlightMorph: function (morph) {
        return new lively.morphic.BoundsMarker()
          .openInWorld()
          .alignWithMorph(morph);
      },

      highlightBounds: function (bounds) {
        return new lively.morphic.BoundsMarker()
          .openInWorld()
          .alignWithRect(bounds);
      },
    });
  }); // end of module
