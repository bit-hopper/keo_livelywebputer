...
'initializing', {
    ...
    setupLogo: function() {
        // Identity-published parts (item.envelope set — see
        // lively.identity.IdentityPartsSpace.createPartItemFromEnvelope)
        // have no WebDAV .html/.svg logo artifact: getHTMLLogoURL() points
        // at a path the identity server never serves (confirmed 404 —
        // those files are only ever generated by the legacy WebDAV publish
        // path), so setupHTMLLogo's fetch fails and silently leaves the
        // icon blank. copyToIdentityPartsSpace (PartsBin.js) now stores the
        // same asHTMLLogo() snapshot WebDAV parts use, directly in the
        // envelope (state.htmlLogo) — render that instead of fetching.
        if (this.partItem && this.partItem.envelope) {
            ...
            if (htmlLogo) {
                ...
                this.setupIdentityPlaceholderLogo();
            }
            this.setupLogoLabel();
            return;
        }
    ...
    // Renders a pre-computed asHTMLLogo() snapshot (stored at publish time,
    // see copyToIdentityPartsSpace) the same way setupHTMLLogo's network
    // response gets rendered — same body-stripping, same mount pattern —
    // just skipping the network round-trip since the markup is already local.
    setupIdentityHTMLLogo: function(htmlLogoSource) {
        var source = htmlLogoSource.replace(/.*\<body\>/, "").replace(/\<\/body\>.*/, "");
        var node = XHTMLNS.create('div');
        try {
            node.innerHTML = source;
        } catch (e) {
            node.innerHTML = '<span>' + this.targetName + '</span>';
        }
    ...
    setupIdentityPlaceholderLogo: function() {
        var node = XHTMLNS.create('div');
        node.style.cssText = 'width:94px;height:94px;display:flex;align-items:center;' +
            'justify-content:center;font-size:36px;font-family:Arial,sans-serif;' +
            'color:#999;background:#f0f0f0;border-radius:4px;box-sizing:border-box;';
        node.textContent = (this.targetName || '?').charAt(0).toUpperCase();
        var morph = new lively.morphic.Morph(new lively.morphic.Shapes.External(node));
        morph.ignoreEvents();
        morph.setBounds(new Rectangle(3,3,94,94));
        this.addMorphBack(morph);
