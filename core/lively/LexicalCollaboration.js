...
Object.extend(lively.LexicalCollaboration = {}, {
  ...
  setupCollaboration: function(lexicalEditor, documentId, options) {
    ...
    const collaboration = {
      ...
      connectToServer: function(serverConfig) {
        this._syncServer = {
          url: serverConfig.url || 'http://localhost:9001/nodejs/CollaborativeEditor/',
      ...
      _startServerSync: function() {
        ...
        }.bind(this), options.syncInterval || 1000);
      ...
      _fetchUpdatesFromServer: function() {
        ...
        url.search = '?docId=' + encodeURIComponent(this._documentId) + 
                     '&stateVector=' + encodeURIComponent(this._getStateVector());

        url.asWebResource().beAsync().withJSONWhenDone(function(result) {
          if (result && result.updates) {
            // Apply updates from server
            const updates = result.updates.map(function(u) {
              return new Uint8Array(atob(u).split('').map(function(c) { 
      ...
      _pushUpdatesToServer: function() {
        ...
        }.bind(this)).post(JSON.stringify(updateData), 'application/json');
      },
      ...
      _observeChanges: function() {
        this._yDoc.on('update', function(update) {
  ...
  _createLexicalBinding: function(lexicalEditor, yXmlFragment) {
    ...
    const yXmlObserver = function(event) {
      ...
      yXmlFragment.insert(0, [
        new yxml.Element('editor-state'),
      ]);
  ...
  loadDocumentFromServer: function(documentId, callback) {
    const url = new URL('http://localhost:9001/nodejs/CollaborativeEditor/load');
  ...
  saveDocumentToServer: function(documentId, state, callback) {
    const url = new URL('http://localhost:9001/nodejs/CollaborativeEditor/save');
