...
(function bootstrapLively(Global) {
  ...
  var BrowserDetector = function (optSpec) {
    ...
    that.detectBrowser = function (optSpec) {
      ...
      if (this.browser === undefined || browser === null) {
        ...
        while (userAgent.indexOf(browserSpec[i]) < 0 && i <= browserCount) {
          i += 1;
        }
        if (i < browserCount) {
    ...
    that.browserSpec = function () {
      if (
        this.browserSpecPointer !== undefined &&
        this.browserSpecPointer >= 0
      ) {
    ...
    that.extractVersion = function (prefix) {
      var uaInfo = userAgent.split(" "),
        ...
        i = 0;
      while (uaInfo[i] && uaInfo[i].indexOf(prefix) < 0) {
        i += 1;
      }
  ...
  (function setupGlobal() {
    if (browserDetector.isNodejs()) {
      ...
      var jsdom = require("jsdom").jsdom,
        ...
        worldData = {
          id: 0,
          registry: { 0: world, 1: shape, isSimplifiedRegistry: true },
        },
        worldDataJson = JSON.stringify(worldData),
        body =
          '<script type="text/x-lively-world">' + worldDataJson + "</script>",
        markup =
          "<html><head><title>Lively</title></head><body>" +
          body +
          "</body></html>",
        doc = jsdom(markup);
  ...
  (function setupConsole() {
    var platformConsole =
        (typeof window !== "undefined" ? window.console : Global.console) || {},
  ...
  var LoadingScreen = {
    ...
    buildBrokenWorldMessage: function () {
      var el = document.createElement("div"),
        text1 = document.createTextNode(
          "An error occurred. " + "If the world does not load " + "you can ",
        ),
    ...
    build: function () {
      var background = this.buildBackground(),
        ...
        consoleButton = this.buildButton(
          "console",
          "LoadingScreen.toggleConsole();",
          "console",
          "right: 170px;",
        ),
        closeButton = this.buildButton(
          "close",
          "LoadingScreen.remove();",
          "close",
          "right: 90px;",
        ),
        moduleDebugButton = this.buildButton(
          "moduleDebug",
          "window.open(lively.moduleDependencyViz());",
          "module dbg",
          "right: 250px;",
        ),
  ...
  Global.JSLoader = {
    ...
    loadAllLibs: function (urlsAndTests, thenDo) {
      ...
      lively.lang.arr.mapAsyncSeries(
        ...
        function tester(urlAndTest, _, next) {
          ...
          lively.lang.fun.waitFor(1000, urlAndTest.test, next);
        },
        function (err) {
    ...
    loadJSON: function (url, onLoadCb) {
      // Note: async API no longer supports beSync parameter
      this.getViaXHR(url)
        .then(function (result) {
          try {
            ...
            console.error("cannot parse JSON from %s: %s", url, e.message);
            onLoadCb && onLoadCb(e, null);
          }
        })
        .catch(function (err) {
          console.warn("cannot load JSON %s: %s", url, err.message);
    ...
    loadViaXHR: function (arg1, arg2, arg3) {
      ...
      // Old: loadViaXHR(loadSync, url, onLoadCb) - 3 parameters
      // New: loadViaXHR(url, onLoadCb) - 2 parameters

      var loadSync, url, onLoadCb;

      // Detect signature based on argument types and count
      if (arguments.length >= 3 || typeof arg1 === "boolean") {
        // Old signature: loadViaXHR(loadSync, url, onLoadCb)
        loadSync = arg1;
        url = arg2;
        onLoadCb = arg3;
      } else if (typeof arg1 === "string") {
        // New signature: loadViaXHR(url, onLoadCb)
        url = arg1;
        onLoadCb = arg2;
      } else {
        console.error(
          "loadViaXHR called with unexpected arguments:",
          arguments,
        );
        return null;
      }

      // Load via XHR using the async fetch-based getViaXHR
      this.getViaXHR(url)
        .then(function (result) {
          Global.JSLoader.evalJavaScriptFromURL(
            url,
            result.responseText,
            onLoadCb,
            result.headers,
        ...
        .catch(function (err) {
          console.warn("cannot load %s: %s", url, err.message);
    ...
    loadViaScript: function (url, onLoadCb) {
      ...
      if (onLoadCb) {
        script.onload = function (evt) {
          onLoadCb && onLoadCb(null);
        };
        script.onerror = function (evt) {
          onLoadCb && onLoadCb(evt);
        };
      }
    ...
    loadCombinedModules: function (combinedFileUrl, callback, hash) {
      ...
      var combinedLoader = {
        ...
        includedInCombinedFile: function (url) {
          return (
            this.expectedModuleURLs && this.expectedModuleURLs.indexOf(url) >= 0
          );
        },
        ...
        loadJs: function (url, onLoadCb, loadSync, okToUseCache, cacheQuery) {
          if (!this.includedInCombinedFile(url)) {
            console.log(
              "load file that is not in" + " combined modules: " + url,
            );
            originalLoader.loadJs(
              url,
              onLoadCb,
              loadSync,
              okToUseCache,
        ...
        isLoading: function (url) {
          return (
            originalLoader.isLoading(url) || this.includedInCombinedFile(url)
          );
        },

        currentDir: function () {
          return originalLoader.currentDir();
        },
      };
        ...
        lively.require(waitForModules).toRun(function () {
          ...
          var allModules = combinedLoader.expectedModules,
            realModules = allModules.select(function (ea) {
              ...
              return (
                !ea.include("lively-libs") &&
                !ea.include("node_modules") &&
                lively.Class.forName(ea) !== undefined
              );
            });
          lively.require(realModules).toRun(function () {
            callback && callback();
          });
        });
      }

      if (this.isLoading(combinedFileUrl)) {
        callCallback();
        return;
      }
    ...
    getLinkAttribute: function (el) {
      return (
        el.getAttributeNS(this.XLINKNamespace, "href") || el.getAttribute("src")
      );
    },
    ...
    isLoading: function (url) {
      ...
      return this.loadedURLs.indexOf(url) > -1 || this.scriptInDOM(url);
    },
    ...
    getOption: browserDetector.isNodejs()
      ? function (option) {
          return undefined; /* TODO: get Node.JS cmd args */
        }
      : function (option) {
    ...
    resolveURLString: function (urlString) {
      ...
      if (urlString.match(/^\/\//)) {
        ...
        ((protocol = protocolMatch[1]), (result = protocolMatch[2]));
      }
      // resolve ..
      do {
        urlString = result;
    ...
    makeUncached: function (urlString, cacheQuery) {
      ...
      return (
        urlString + (urlString.indexOf("?") === -1 ? "?" : "&") + cacheQuery
      );
    },
    ...
    getViaXHR: function (beSync, url, callback) {
      // Supports both old and new signatures for backwards compatibility
      // Old: getViaXHR(beSync, url, callback) - where beSync is boolean
      // New: getViaXHR(url) or getViaXHR(url, callback) - where first arg is string (URL)

      // Normalize arguments based on what was actually passed
      // If first argument looks like a URL (string), use new signature
      if (typeof beSync === "string") {
        // First argument is a string, so treat it as the URL (new signature)
        if (typeof url === "function") {
          // getViaXHR(url, callback)
          callback = url;
        } else if (typeof url === "boolean" || typeof url === "undefined") {
          // getViaXHR(url) called without callback
          callback = undefined;
        }
        url = beSync; // The string URL is our real URL
        beSync = undefined;
      }
      // else: Old signature, beSync is boolean, url and callback are correct

      // Validate URL is a string
      if (typeof url !== "string") {
        var error = new Error("Invalid URL passed to getViaXHR");
      ...
      var makeRequest = function () {
        return fetch(url).then(function (response) {
          var headerObj = {};
          // Convert Headers object to plain object (case-insensitive)
          response.headers.forEach(function (value, key) {
            headerObj[key] = value;
            // Add lowercase version for Safari compatibility
      ...
      if (typeof callback === "function") {
        makeRequest()
          .then(function (result) {
            // Old signature expects: callback(err, responseText)
            callback(null, result.responseText);
          })
          .catch(function (error) {
            console.error("XHR request failed for " + url + ":", error);
            // Old signature expects: callback(err, null)
            callback(error, null);
          });
        return null;
      }
  ...
  var libsFile =
      /*useMinifiedLibs ? 'core/lib/lively-libs.js' :*/ "core/lib/lively-libs-debug.js",
    libsFiles = [libsFile],
    ...
    rootPath = (function findRootPath() {
      ...
      if (Global.Config && Config.standAlone) {
        ...
        return (Config.rootPath = document.URL.substring(
          0,
          url.lastIndexOf("/") + 1,
        ));
      }
  ...
  Global.LivelyLoader = {
        ...
        if (err) ldr.handleStartupError(err);
          ...
          ldr.bootstrap(function (err) {
            if (err) ldr.handleStartupError(err);
              ...
              ldr.loadMain(document, function (err) {
                if (err) ldr.handleStartupError(err);
    ...
    bootstrap: function (thenDoFunc) {
      ...
      // Use async IIFE to enable await within the callback context
      (async function () {
        var combinedModulesHash = null;

        if (optimizedLoading) {
          var hashUrl = base + "generated/combinedModulesHash.txt";
          try {
            var result = await loader.getViaXHR(hashUrl);
            combinedModulesHash = result.responseText;
            console.log("optimized loading hash retrieved");
          } catch (err) {
            console.warn("Optimized loading not available: " + err.message);
          }
        }

        // Load modules immediately after hash retrieval (async, non-blocking)
        // The fetch-based getViaXHR already provides async/await support without blocking
        if (combinedModulesHash) {
          console.log("optimized loading enabled");
          var combinedModulesUrl =
  ...
  Global.EmbededLoader = {
    ...
    embedAndLoadWorld: function (worldURL, targetElement) {
      ...
      if (canvas.getElementsByTagName("g").length > 0) {
        ...
        canvas.setAttribute(
          "width",
          world.width.baseVal.value.toString() + "px",
        );
        canvas.setAttribute(
          "height",
          world.height.baseVal.value.toString() + "px",
        );
      } else {
        // FIXME!!!
        Config.resizeScreenToWorldBounds = false;
        ...
        var elementHeight = targetElement.clientHeight;
    ...
    embedLively: function () {
      ...
      var ownUrl = document.location.href,
        url =
          ownUrl.substring(0, ownUrl.lastIndexOf("/") + 1) +
  ...
  var LivelyMigrationSupport = (Global.LivelyMigrationSupport = {
    ...
    extractMigrationLevel: function (doc) {
      ...
      var node = doc.getElementById(
        LivelyMigrationSupport.migrationLevelNodeId,
      );
      return node ? Number(node.textContent) : 0;
    },

    setDocumentMigrationLevel: function (doc) {
      LivelyMigrationSupport.documentMigrationLevel =
        LivelyMigrationSupport.extractMigrationLevel(doc);
    },
    ...
    applyWorldJsoTransforms: function (jso) {
      LivelyMigrationSupport.worldJsoTransforms.forEach(function (func) {
        jso = func(jso);
      });
      return jso;
    },
  ...
  function initNodejsBootstrap() {
    // remove libs, JSON:
    Global.LivelyLoader.bootstrapFiles = ["lib/lively-libs-nodejs.js"].concat(
  ...
  Global.lively.ApplicationCache = {
    ...
    setupAppCacheHandlers: function (remove) {
      ...
      console.log(
        "%s appcache event handlers...",
        remove ? "Uninstalling" : "Installing",
      );
      [
        "checking",
        "downloading",
        ...
        var handlerName =
          "on" + evtName.charAt(0).toUpperCase() + evtName.slice(1);
        var handler =
          this["_" + handlerName] ||
          (this["_" + handlerName] = this[handlerName].bind(this));
    ...
    onChecking: function (evt) {
      var msg =
        "Application cache is checking if there are new sources to load...";
      console.log(msg);
    ...
    onUpdateready: function (evt) {
      ...
      var msg =
        "A newer version of Lively is available.\n" +
    ...
    onObsolete: function (evt) {
      ...
      lively.whenLoaded(function (world) {
        var msg =
          "It appears that the Lively server is not\n" +
