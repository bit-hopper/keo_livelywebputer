  ...
  var readSyncMessage = (decoder, encoder, doc4, transactionOrigin, errorHandler) => {
    ...
    switch (messageType) {
      ...
      case messageYjsUpdate:
        readUpdate2(decoder, doc4, transactionOrigin, errorHandler);
        break;
      default:
...
${reason}`);
  ...
  var ReplaceStep = class _ReplaceStep extends Step {
    ...
    */
    ...
    map(mapping) {
      ...
      let from3 = this.from == this.to && _ReplaceStep.MAP_BIAS < 0 ? to : mapping.mapResult(this.from, 1);
    ...
    if ($from.parent.isTextblock && $to.parent.isTextblock && $from.start() != $to.start() && $from.parentOffset == 0 && $to.parentOffset == 0) {
      ...
      for (let d = $to.depth; d > shared; d--)
        if ($to.node(d).type.spec.isolating)
          isolated = true;
      if (!isolated) {
        for (let d = $from.depth; d > 0 && from3 == $from.start(d); d--)
          from3 = $from.before(d);
        for (let d = $to.depth; d > 0 && to == $to.start(d); d--)
          to = $to.before(d);
        $from = tr.doc.resolve(from3);
  ...
  var Transform = class {
    ...
    */
    changedRange() {
      let from3 = 1e9, to = -1e9;
      for (let i = 0; i < this.mapping.maps.length; i++) {
        let map4 = this.mapping.maps[i];
        if (i) {
          from3 = map4.map(from3, 1);
          to = map4.map(to, -1);
        }
        map4.forEach((_f, _t2, fromB, toB) => {
          from3 = Math.min(from3, fromB);
          to = Math.max(to, toB);
        });
      }
      return from3 == 1e9 ? null : { from: from3, to };
    }
    /**
    @internal
    */
    ...
    */
    insertText(text2, from3, to) {
      ...
      if (from3 == null) {
        ...
        if (!this.selection.empty && this.selection.to == from3 + text2.length)
        ...
        if ($cursor) {
          ...
          for (let i = 0; i < ranges.length; i++) {
            ...
            if (!add2) {
              ...
              tr.addMark(from3, to, markType.create(attrs));
            }
          }
          dispatch(tr.scrollIntoView());
        }
  ...
  function wrapDispatchForJoin(dispatch, isJoinable) {
    return (tr) => {
      ...
      for (let i = 0; i < tr.mapping.maps.length; i++) {
        let map4 = tr.mapping.maps[i];
        for (let j = 0; j < ranges.length; j++)
          ranges[j] = map4.map(ranges[j]);
        map4.forEach((_s, _e, from3, to) => ranges.push(from3, to));
      }
      let joinable3 = [];
      for (let i = 0; i < ranges.length; i += 2) {
        let from3 = ranges[i], to = ranges[i + 1];
        let $from = tr.doc.resolve(from3), depth = $from.sharedDepth(to), parent = $from.node(depth);
  ...
  function splitListItemKeepMarks(itemType, itemAttrs) {
    ...
    return (state, dispatch) => {
      return split2(state, dispatch && ((tr) => {
        let marks = state.storedMarks || state.selection.$to.parentOffset && state.selection.$from.marks();
        if (marks)
          tr.ensureMarks(marks);
        dispatch(tr);
  ...
  // build/postcard-deps-entry.js
  if (!window.PM) {
    window.Y = yjs_exports;
    window.WebsocketProvider = WebsocketProvider;
    window.yProsemirror = { ySyncPlugin, yUndoPlugin, yDocToProsemirrorJSON };
    window.PM = { model: dist_exports, state: dist_exports2, view: dist_exports3, commands: dist_exports4, keymap: dist_exports5, history: dist_exports6, schemaList: dist_exports7 };
  }
})();
